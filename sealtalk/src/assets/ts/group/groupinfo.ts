...
groupInfo.controller("groupinfoController", ["$scope", "$rootScope", "$state", "$stateParams", "mainDataServer", "mainServer", "RongIMSDKServer",
    function($scope: any,$rootScope: any, $state: angular.ui.IStateService, $stateParams: angular.ui.IStateParamsService, mainDataServer: mainDataServer, mainServer: mainServer, RongIMSDKServer: RongIMSDKServer) {
        ...
        $scope.quitGroup = function() {
        	var chgroupid = this.groupInfo.ida;
alert(chgroupid)
            mainServer.group.quit(chgroupid).success(function(rep) {
                if (rep.code == 1) {
        ...
        $scope.uploadfile = function() {
        	var file = <HTMLInputElement>document.getElementById('communityFile');
        	var files : any =file.files;
        	console.log(this.groupInfo.ida);
        	mainServer.group.upLoad(files[0].name,this.groupInfo.ida,"AA").success(function(rep){
        		console.log(rep);
        	})
        ...
        $scope.toChat = function(ida:string) {
        	//alert(1);
        	//console.log(this.groupInfo);
        	//console.log(webimmodel.conversationType);
        	//1: "Private", 2: "Discussion", 3: "Group", 4: "ChartRoom"
        	if(this.groupInfo.imtype == 1){
            	$state.go("main.chat", { targetId: $scope.groupInfo.id, targetType: webimmodel.conversationType.Group}, { location: "replace" });
        	}else if(this.groupInfo.imtype == 2){
        		RongIMLib.RongIMClient.getInstance().joinChatRoom(this.groupInfo.id, 50, {
				  onSuccess: function() {
				  		console.log("加入聊天室成功");
            	$state.go("main.chat", { targetId: $scope.groupInfo.id, targetType: webimmodel.conversationType.ChartRoom}, { location: "replace" });
				       // 加入聊天室成功。
				  },
        ...
        $scope.dismiss = function() {
        	var chgroupid = this.groupInfo.ida;
            //解散群组
            mainServer.group.dismissGroup(chgroupid).success(function(rep) {
                if (rep.code == 1) {
                    mainDataServer.contactsList.removeGroup(groupid);
                    RongIMSDKServer.removeConversation(webimmodel.conversationType.Group, groupid).then(function() {
                        setTimeout(function () {
