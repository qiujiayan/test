    ...
    function ($scope: any, $state: angular.ui.IStateService, $stateParams: any, $window: angular.IWindowService,
        ...
        function setPortrait() {
            if ($scope.user.id) {
                ...
                angular.element(document.getElementById("portrait")).css("background-color", "#000")
            }
            // console.log("这是什么东东" + $scope.user.id)
        }
        ...
        $scope.removeFriend = function () {
            ...
            mainServer.friend.delete($scope.user.id).success(function () {
                ...
                // alert("1111");
                var _Str = String($scope.user.id);
                // console.log("66666666666666666" + webimmodel.conversationType.Private + "-" + _Str);
                RongIMSDKServer.removeConversation(webimmodel.conversationType.Private, _Str).then(function () {
                    // alert("2222");
                    loading = false;
