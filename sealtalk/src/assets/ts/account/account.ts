    ...
    function ($scope: any, $state: angular.ui.IStateService, mainServer: mainServer, mainDataServer: mainDataServer, conversationServer: conversationServer, RongIMSDKServer: RongIMSDKServer) {
        ...
        $scope.signin = function () {
            ...
            if ($scope.formSignin.$valid) {
                mainServer.user.signin($scope.user.accountNumber, "1", $scope.user.passWord).success(function (rep) {
                    console.log(rep);
                    if (rep.code === 1) {
                        // 登录账户
                        mainDataServer.loginUser.id = rep.content.userId;
                        mainDataServer.loginUser.token = rep.content.ryToken;

                        // localStorage.setItem("userToken",rep.content.token);
                        //                      mainDataServer.loginUser.id = rep.result.id;
                        //                      mainDataServer.loginUser.token = rep.result.token;
                        var exdate = new Date();
                        exdate.setDate(exdate.getDate() + 30);
                        webimutil.CookieHelper.setCookie("loginuserid", rep.content.userId, exdate.toGMTString());
                        webimutil.CookieHelper.setCookie("loginusertoken", rep.content.ryToken, exdate.toGMTString());//融云token
                        webimutil.CookieHelper.setCookie("loginselftoken", rep.content.token, exdate.toGMTString());//自己的token

                        webimutil.CookieHelper.setCookie("userselfname", rep.content.username, exdate.toGMTString());
                        webimutil.CookieHelper.setCookie("userselfavater", rep.content.avatar, exdate.toGMTString());
...
account.controller("signupController", ["$scope", "$interval", "$state", "mainServer",
    function ($scope: any, $interval: angular.IIntervalService, $state: angular.ui.IStateService, mainServer: mainServer) {
        ...
        $scope.signup = function () {
            ...
            // console.log(11111);
            // console.log(mainServer.user);
            // return false;
            if ($scope.formSignup.$valid) {
                // mainServer.user.verifyCode($scope.user.phone, "86", $scope.user.code).success(function (data) {
                    // mainServer.user.signup($scope.user.phone, "86", $scope.user.code).success(function(data) {    
                    // console.log(data.code)
                    // if (data.code == 200 && data.result.verification_token) {
                        console.log($scope.user);

                        mainServer.user.signup($scope.user.phone.toString(), $scope.user.code.toString(), $scope.user.passWord.toString(),  $scope.user.invitationCode.toString()).
                            success(function (e) {
