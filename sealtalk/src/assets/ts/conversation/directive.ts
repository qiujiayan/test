...
conversationDire.directive("imageMessage", [function() {
    return {
        ...
        '<span id="{{\'rebox_\'+$id}}" class="Message-entry gallery" style="">' +
...
//          console.log(scope.item);
//          console.log(scope.item.imageViewUrl);
...
conversationDire.directive("videoMessage", [function() {
    return {
        restrict: "E",
        scope: { item: "=" },
        template: '<div class="" id={{itemid}}>' +
        '<div class="Message-img">' +
        '<span id="{{\'rebox_\'+$id}}" class="Message-entry gallery" style="position:relative;">' +
        '<img ng-src="{{\'./assets/img/play.png\'}}" class="Message-img-playbtn" style="width:50px;height:50px;position:absolute;left:50%;top:50%;margin-left:-25px;margin-top:-25px;">'+
        '<a><img ng-src="{{item.videoPicture}}"  data-image="{{item.videoPicture}}" alt=""/></video></a>' +
        '</span>' +
        '<a href="{{item.videoUrl}}" download>下载播放</a>' +
        '</div>' +
        '</div>',
        link: function(scope: any, ele: angular.IRootElementService, attr: any) {
            var img = new Image();

            //console.log(scope.item.videoPicture);
            scope.itemid = scope.$parent.item.messageUId;
            img.src = scope.$parent.item.content.videoPicture;
...
//这块代码不执行
conversationDire.directive("videoMsg", [function() {//videoMsg
    ...
    return {
        ...
        '<span id="{{\'rebox_\'+$id}}" class="Message-entry gallery" style="position:relative;">' +
        '<img ng-src="{{\'./assets/img/play.png\'}}" class="Message-img-playbtn" style="width:50px;height:50px;position:absolute;left:50%;top:50%;margin-left:-25px;margin-top:-25px;">'+
        '<a><img ng-src="{{item.mExtra}}"  data-image="{{item.mExtra}}" alt=""/></video></a>' +
        '</span>' +
        '<a href="{{item.mMediaUrl}}" download>下载播放</a>' +
        '</div>' +
        '</div>',
        ...
        link: function(scope: any, ele: angular.IRootElementService, attr: any) {
            ...
            scope.itemid = scope.$parent.item.messageUId;
            img.src = scope.$parent.item.content.mExtra;
...
//     // '<!-- <p>发给您一张示意图</p> -->' +
//      // '<img ng-src="{{item.content||\'../../static/images/barBg.png\'}}" data-image="{{item.imageUri}}" alt=""/>' +
//      '<a><img ng-src="{{item.content||\'../../static/images/barBg.png\'}}"  data-image="{{item.imageUri}}" alt=""/></a>' +
////      '<a href="{{item.imageUri||\'assets/img/barBg.png\'}}"><img ng-src="{{item.content||\'../../static/images/barBg.png\'}}"  data-image="{{item.imageUri}}" alt=""/></a>' +
//      '</span>' +
//       //'<a download>下载</a>' +
...
//conversationDire.directive("imageMessageT", [function() {
//  return {
//      restrict: "E",
//      scope: { item: "=" },
//      template: '<div class="" id={{itemid}}>' +
...
//     // '<!-- <p>发给您一张示意图</p> -->' +
//      // '<img ng-src="{{item.content||\'../../static/images/barBg.png\'}}" data-image="{{item.imageUri}}" alt=""/>' +
//      '<a><img ng-src="{{item.content||\'../../static/images/barBg.png\'}}"  data-image="{{item.imageViewUrl}}" alt=""/></a>' +
////      '<a href="{{item.imageUri||\'assets/img/barBg.png\'}}"><img ng-src="{{item.content||\'../../static/images/barBg.png\'}}"  data-image="{{item.imageUri}}" alt=""/></a>' +
//      '</span>' +
//       //'<a download>下载</a>' +
//      '<a href="{{item.imageViewUrl}}" download>下载</a>' +
//      '</div>' +
//      '</div>',
//      link: function(scope: any, ele: angular.IRootElementService, attr: any) {
//          var img = new Image();
//          console.log(scope);
//          scope.itemid = scope.$parent.item.messageUId;
//          img.src = scope.item.imageViewUrl;
////          setTimeout(function() {
////              $('#rebox_' + scope.$id).rebox({ selector: 'a' }).bind("rebox:open", function() {
////                  //jQuery rebox 点击空白关闭
////                  var rebox = <any>document.getElementsByClassName("rebox")[0];
...
//          img.onload = function() {
//              scope.$apply(function() {
//                  scope.item.content = scope.item.imageViewUrl
//              });
//          }
//          scope.showBigImage = function() {
//				
...
conversationDire.directive("getHistoryMessage", [function() {
    return {
        restrict: "E",
        scope: { myclick: "&" },
        template: '<div class="Messages-getHistory"><b ng-click="myclick()" class="" style="cursor: pointer;"></b></div>',
