...
function adjustScrollbars() {
    ...
    ele.style.height = document.documentElement.clientHeight -
        ...
        parseFloat(getComputedStyle(document.querySelector('.box_hd')).height) - 40 + "px";
...
conversationCtr.controller("conversationController", ["$scope", "$state", "mainDataServer", "conversationServer", "mainServer", "RongIMSDKServer", "$http", "$timeout", "$location", "$anchorScroll",
    function ($scope: any, $state: angular.ui.IStateService, mainDataServer: mainDataServer, conversationServer: conversationServer, mainServer: mainServer, RongIMSDKServer: RongIMSDKServer, $http: angular.IHttpService, $timeout: angular.ITimeoutService, $location: angular.ILocationService, $anchorScroll: angular.IAnchorScrollService) {
        ...
        //console.log(targetType);//1代表个人 3群组 4是聊天室
        //xyy
        if (targetType == 3 || targetType == 4) {
            //      console.log($scope);
            $scope.groupInfo = mainDataServer.contactsList.getGroupById(targetId);
            //console.log($scope.groupInfo);

            //	        if($scope.groupInfo.unSpeak == 1){
            //	        	$('#message-content').css('display','none');
            //	        	$('.sendBtn').html("群主已设置全群禁言");
            //	        }

            var aa = $scope.groupInfo.ida;
            //console.log(aa);
            mainServer.group.getGroupDetail(aa).success(function (rep) {
                //console.log(rep);
                if (rep.content.unSpeak == 1 && rep.content.isAdmin != 1) {
                    $('#message-content').css('display', 'none');
                    $('#MessageForm-header').css('display', 'none');
                    $('.sendBtn').html("群主已设置全群禁言");
                }
                //	    		if(rep.content.imType == 2){
                //	    			var urls=window.location.href;
                //	    			urls = urls.substr(0,urls.length-1) + '4';
                //	    			//console.log(urls);
                //	    			window.location.href = urls;
                //	    			RongIMLib.RongIMClient.getInstance().joinChatRoom(this.groupInfo.imGroupId, 50, {
                //					  onSuccess: function() {
                //					  		console.log("加入聊天室成功");
                //					       // 加入聊天室成功。
                //					  },
        ...
        RongIMSDKServer.getConversation(targetType, targetId).then(function (data) {
            ...
            //console.log(RongIMSDKServer.getDraft(targetType, targetId));
            ...
            if (targetType == webimmodel.conversationType.Private) {
                ...
                if (friend) {
                    mainServer.friend.getProfile(targetId).success(function (data) {
                        var f = new webimmodel.Friend({ id: data.content.friendUserId, name: data.content.name, imgSrc: data.content.avatar });
                        f.displayName = data.content.name;
                        f.mobile = data.content.no;
                        ...
                        mainDataServer.conversation.updateConversationDetail(targetType, targetId, data.content.name || data.content.name, data.content.avatar);
                        conversationServer.updateHistoryMessagesCache(targetId, targetType, data.content.name || data.content.name, data.content.avatar);
                    ...
                    //  mainServer.user.getInfoS(targetId).then(function(rep) {
                    //      $scope.user.id = rep.content.userInfo.userFriendUserId
                    //      $scope.user.nickName = rep.content.userInfo.name
                    //      $scope.user.portraitUri = rep.content.userInfo.avatar;
                    //
                    //      $scope.user.firstchar = webimutil.ChineseCharacter.getPortraitChar(rep.content.userInfo.name);
                    //      setPortrait();
                    //  })
                }
        ...
        function addmessage(msg: webimmodel.Message) {
            ...
            if (msg.messageType == webimmodel.MessageType.ImageMessage || msg.messageType == webimmodel.MessageType.ImageViewMessage) {
                setTimeout(function () {
        ...
        $scope.sendBtn = function () {
            ...
            //发送消息
            // var msg = RongIMLib.TextMessage(con,user);
            // var msg = new RongIMLib.TextMessage({content:"hello RongCloud!",extra:"附加信息"});

            var msg = new RongIMLib.TextMessage({content:con,user:user});
            ...
            //console.log(targetType);
            RongIMSDKServer.sendMessage(targetType, targetId, msg, atFlag && (targetType == webimmodel.conversationType.Group
                || targetType == webimmodel.conversationType.ChartRoom)).then(function (msg) {
                    //RongIMSDKServer.sendMessage(targetType, targetId, msg, atFlag).then(function(msg) {
                    ...
                    switch (error.errorCode) {
                        ...
                        case RongIMLib.ErrorCode.NOT_IN_GROUP:
                            ...
                            break;
                        default:
                        //  webimutil.Helper.alertMessage.error("消息发送失败,错误代码0 " + error.errorCode, 2);
                    }
                    // console.log('消息发送失败,错误代码1:' + error.errorCode);
                    if (content) {
        ...
        //xyy上传社群文件页面
        $scope.upCommunityFiles = function () {
            //console.log($scope.groupInfo);
            var imGroupId = $scope.groupInfo.id;
            var communityId = $scope.groupInfo.ida;

            var filetoken = webimutil.CookieHelper.getCookie("loginselftoken");
            var pUrl = "im=" + imGroupId + "&comm=" + communityId + "&filet=" + filetoken;

            localStorage.setItem("filetoken", filetoken);
            window.open("https://www.chaohuo.net/chtalk/communityfile/index.html?" + pUrl);
            //window.open("http://192.168.0.250:8020/rongYun/sealtalk-web-master/src/communityfile/index.html?"+pUrl);
        }
        ...
        RongIMLib.RongUploadLib.getInstance().setListeners({
            ...
            onFileUploaded: function (file: any, message: webimmodel.Message) {
                ...
                if (message.messageType == webimmodel.MessageType.ImageMessage || message.messageType == webimmodel.MessageType.ImageViewMessage) {
