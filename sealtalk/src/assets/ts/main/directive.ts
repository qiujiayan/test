...
mainDire.directive("conversation", ["$state", "mainDataServer", function($state: angular.ui.IStateService, mainDataServer: mainDataServer) {
    return {
        ...
        // ng-show="item.title"!=""主要是不让空的对话框显示出来
        template: '<div class="chatList" ng-show="item.title"!="" ng-class="{selected:isCurrentConversation}" id="{{item.targetType}}_{{item.targetId}}">' +
