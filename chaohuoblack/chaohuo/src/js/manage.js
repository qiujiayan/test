		   ...
		   checkBanner((this.__index)-1,type,10,status,function(reqs){
				...
				for(var i = 0;i < tableLength.length;i++){
					...
					oTableTr += `<tr>
								...
								<td class="banner-time">${formatDate(new Date(tableLength[i].createTime))}</td>
...
//				//console.log($(aa[16]));
	    ...
	    $('.banner-add').on('click',function(){
	    	linkUrl = $('.banner-link').val();
	    	show = $("select[name='bannershow']").val();
	    	sourceUrl = sourceUrl;
		    //console.log(sourceUrl);
	    	status =$("select[name='bannerstatus']").val();
		    ...
		    addBanner(linkUrl,show,sourceUrl,status,type,function(reqs){
		    	if(reqs.code == 1){
		    		alert("上传成功");
		    		location.reload();
